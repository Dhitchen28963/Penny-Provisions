...
class DateEncoder(json.JSONEncoder):
    """
    Extends json.JSONEncoder to serialize date objects into ISO format.
    """

    def default(self, obj):
        """
        Override the default method to handle date objects.
        """
...
class PennyProvisions:
        ...
        try:
            with open(self.user_data_file, 'r') as file:
                ...
                for _username, user_info in data.items():
                    if "target_date" in user_info and isinstance(
                            user_info["target_date"], str
                    ):
                        user_info["target_date"] = datetime.strptime(
                            user_info["target_date"], "%Y-%m-%d"
                        ).date()
    ...
    def authenticate_user(self):
        ...
        while True:
            ...
            if (username in self.user_data and
                ...
                print("Invalid credentials. Hit enter to try again")
    ...
    def account_creation(self):
        ...
        Once the account is created, the user will need to log in.
        """
    ...
    def view_current_savings(self, username):
        ...
        """
        user_currency = self.user_data[username].get("currency", ('GBP'))
    ...
    def get_valid_amount(self, prompt):
        ...
        In the event a negative amount is inputted, the user is prompted.
        """
        while True:
            try:
    ...
    def select_savings_goal(self, username):
        ...
        for i, (goal_name, goal_info) in enumerate(
            savings_goals.items(), start=1
        ):
            print(f"{i}. {goal_name} - Target Amount: "
                  f"{user_currency[0]}{goal_info['target_amount']:.2f}")

        choice = input("Select savings goal (or type 'cancel' to go back):\n")
        ...
        try:
            ...
            if 1 <= choice_index <= len(savings_goals):
                ...
                print("Invalid choice. Please choose an option listed.")
                return None
        except ValueError:
    ...
    def record_income(self, username, selected_goal):
        """
        Records inputted income data ensuring a valid amount is entered.
        ...
        while True:
            ...
            choice = input("Select an option (or type 'done' to finish):\n")
            ...
            if choice in self.INCOME_OPTIONS:
                ...
                try:
                    income_amount = self.get_valid_amount(
                        f"Enter the amount for {income_type}: "
                    )
                    income_date_str = input(
                        "Enter date of income (YYYY-MM-DD):\n"
                    )
                    income_date = datetime.strptime(
                        income_date_str, "%Y-%m-%d"
                    ).date()
                    ...
                    self.user_data[username]["current_savings"] += \
                        income_amount
                    self.user_data[username]["savings_goals"][selected_goal][
                        "current_savings"
                    ] += income_amount

                    # Capture income details including date and savings goal
                    income_details = {
                        "type": income_type,
                    ...
                    self.save_user_data()
                    currency_symbol = user_currency[0]
                    current_savings = self.user_data[username][
                        'current_savings']
                    formatted_savings = (
                        f"{currency_symbol}{current_savings:.2f}"
                    )
                    print(
                        f"Income recorded successfully! Current savings: "
                        f"{formatted_savings}"
                    )
                except ValueError as e:
                    print(f"Error: {e}. Please enter a valid positive number.")
            else:
                print("Invalid choice. Please select an option listed above.")

    def record_expense(self, username):
        """
        Records inputted expense data ensuring a valid amount is entered.
        ...
        while True:
            ...
            expense_choice = input("Select the type of expense:\n")
            ...
            if expense_choice in self.EXPENSE_OPTIONS:
                ...
                try:
                    expense_amount = self.get_valid_amount("Enter amount for "
                                                           f"{expense_type}: ")
                    expense_date_str = input("Enter the date of the expense "
                                             "(YYYY-MM-DD):\n")
                    expense_date = datetime.strptime(
                        expense_date_str, "%Y-%m-%d"
                    ).date()

                    # Date validation check
                    if expense_date > datetime.now().date():
                        print("Error: Cannot record expense for future date.")
                        continue
                    ...
                    self.user_data[username].setdefault("expenses", []).append(
                        expense_details
                    )

                    self.save_user_data()
                    print(
                        f"expense recorded successfully! Total expense: £"
                        f"{self.calculate_total_expense(username):.2f}"
                    )
                    # Ask the user if they want to add another expense
                    add_another = input("Add another expense? (yes/no):\n")
                    if add_another.lower() != 'yes':
                        break
                except ValueError as e:
                    print(f"Error: {e}. Please enter a valid positive number.")
            else:
                print("Invalid choice. Please select from the options listed.")

    def add_debt_expense_to_savingsgoal(self, username):
        """
        Records inputted debt data ensuring a valid amount is entered.
        ...
        while True:
            ...
            try:
                debt_amount = self.get_valid_amount(
                    f"Enter amount of {debt_type}: "
                )
                debt_name = input(f"Enter a name for the {debt_type}:\n")
                debt_date_str = input("Enter date of the debt (YYYY-MM-DD):\n")
                ...
                self.user_data[username].setdefault("debts", [])\
                    .append(debt_details)

                self.save_user_data()
                print(
                    f"Debt recorded successfully! Total debt: £"
                    f"{self.calculate_total_debt(username):.2f}"
                )

                # Ask the user if they want to add another debt
                add_another = input("Add another debt? (yes/no):\n")
                if add_another.lower() != 'yes':
                    break
            except ValueError as e:
                print(f"Error: {e}. Please enter a valid positive number.")
    ...
    def calculate_total_expense(self, username):
        ...
        total_expense = sum(
            expense["amount"]
            for expense in self.user_data[username].get("expenses", [])
        )
        return total_expense

    def calculate_recommendation(self, username, selected_goal):
        """
        Calculate and display saving recommendation for selected savings goal.
        Collates information about selected savings goal including:
        ...
        If the savings target is reached message is displayed to confirm this.
        """
        user_currency = self.user_data[username].get("currency", ('GBP,'))
        ...
        if current_savings >= target_amount:
            print(
                f"\nCongratulations! Your savings goal '{selected_goal}' "
        ...
        if target_amount > 0 and remaining_days:
            ...
            print(f"\nRecommendations to save ({user_currency_symbol}):")
            print(
                f"{user_currency[0]}{max(weekly_recommendation, 0):.2f} p.w"
            )
            print(
                f"{user_currency[0]}{max(monthly_recommendation, 0):.2f} p.m"
            )
            print(
                f"{user_currency[0]}{max(annually_recommendation, 0):.2f} p.y"
            )
    ...
    def view_all_savings_goals(self, username):
        ...
        user_currency_symbol = user_currency[1]  # Get the currency
        print(f"\nAll Savings Goals ({user_currency_symbol}):")
        for goal_name, goal_info in savings_goals.items():
            print(f"\nGoal: {goal_name}")
            print(f"Goal: {user_currency[0]}{goal_info['target_amount']:.2f}")
            print(f"Target Date: {goal_info.get('target_date', 'N/A')}")
            print(
                f"Savings: {user_currency[0]}"
                f"{goal_info['current_savings']:.2f}"
            )
            ...
            if expenses:
                print(
                    f"    - {expense['type']}: "
                    f"{user_currency[0]}{expense['amount']:.2f} "
                    f"on {expense['date']}"
                )
    ...
    def analyze_monthly_activity(self, username):
        """
        Displays the month with the most expenses and savings.
        """
        ...
        for activity in all_activities:
            ...
            if isinstance(activity["date"], date):
                ...
                month_year = datetime.strptime(activity["date"], "%Y-%m-%d") \
                   .strftime("%Y-%m")
            else:
        ...
        max_income_month = max(
            monthly_totals,
            key=lambda x: monthly_totals[x]["income"],
            default=None
        )
        max_expense_month = max(
            monthly_totals,
            key=lambda x: monthly_totals[x]["expense"],
            default=None
        )
        ...
        if max_income_month:
            print(
                f"Month with the highest income: {max_income_month} - \n"
                f"£{monthly_totals[max_income_month]['income']:.2f}"
            )
        else:
        ...
        if max_expense_month:
            print(
                f"Month with the highest expense: {max_expense_month} - \n"
                f"£{monthly_totals[max_expense_month]['expense']:.2f}"
            )
        else:
    ...
    def add_debt(self, username):
        ...
        while True:
            try:
                debt_amount = self.get_valid_amount("Enter debt amount: ")
                debt_name = input("Enter a name for the debt:\n")
                debt_type = input("Enter the type of debt:\n")

                # Term is when the debt needs to be paid off by
                debt_term = int(input("Enter the term of debt (in months):\n"))
                interest_rate = float(input("Enter interest rate for debt:\n"))
                ...
                self.user_data[username].setdefault("debts", []).append(
                    debt_details
                )

                self.save_user_data()
                print(f"New total £{self.calculate_total_debt(username):.2f}")
    ...
    def view_all_debts(self, username):
        ...
        for debt in debts:
            ...
            remaining_balance = self.calculate_remaining_bal(debt)
    ...
    def add_repayment_to_debt(self, username):
        ...
        while True:
            ...
            for i, debt in enumerate(debts, start=1):
                print(
                    f"{i}. {debt['name']} - Type: {debt['type']} "
                    f" - Remaining Term: {debt['term']} months"
                )
            choice = input("Select the debt (or type 'cancel' to go back):\n")
            ...
            try:
                ...
                if 1 <= choice_index <= len(debts):
                    ...
                    expense_amount = self.get_valid_amount(
                        f"Enter expense amount {debt_name}: "
                    )

                    # Record the expense details including the date
                    expense_date_str = input(
                    ...
                    if expense_date > datetime.now().date():
                        print("Error: Cannot record future dated expense.")
                        continue

                    # Update the debt details with the expense
                    selected_debt["payments"].append(
                    ...
                    print(
                        ...
                        f"Remaining debt on {debt_name}: "
                        f"£{self.calculate_remaining_bal(selected_debt):.2f}"
                    )
                else:
    ...
    def calculate_remaining_bal(self, debt):
        """
        Calculates the remaining balance on a debt.
        """
        # Calculate the total amount paid towards the debt
        payment_amounts = (
            payment["amount"]
            for payment in debt.get("payments", [])
        )
        amount_paid = sum(payment_amounts)

        # Calculate the remaining balance of the debt
        remaining_balance = (
            debt["amount"] +
            (debt["amount"] * debt["interest_rate"] / 100) -
            amount_paid
        )
                ...
                if username:
                    ...
                    while True:
                        ...
                        if option == "1":
                            ...
                            if selected_goal:
                                print(f"Savings '{selected_goal}' selected.")
                        elif option == "3":
                            if selected_goal:
                            ...
                            confirm_logout = input("Are you sure? (yes/no):\n")
                            ...
                            print("Invalid. Please enter number from 1 to 10.")
